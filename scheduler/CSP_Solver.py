#Algorithm for making a CSP and moreover for solving it 
import random
import copy
...
class CSP(object):
        def __init__(self):
                self.nodes = []
                #describes the domain of values assinable to the node
                self.nodeDomains = {}
                #constraints depending only on a singular node, evaluated on all possible domain values
                self.unary_constraints = {}
                #binary constraints depending on node pair, evaluated on all possible domain values
        ...
        def update_binary_constraint_table(self,nodeA,nodeB,tableFactor):
                ...
                for i in tableFactor:
                        for j in tableFactor[i]:
                                assert i in currentTable and j in currentTable[j]
...
class minConflicts(object):
        ...
        #assins each variable a random domain value
        ...
        def conflictedNeighbors(self,assignments,n):
                conflicted = []
                val = assignments[n]
                csp = self.csp  
                soft_weight = 1  #based on number of soft-constraint satisfying neighbors
                #check for KeyError on unary and binary constraints
                try: 
                        if csp.unary_constraints[n][val] == 0: conflicted.append(n)
                except KeyError:
                        pass
                try:    
                        neighbors = set(csp.binary_constraints[n].keys())
                ...
                for m in neighbors:
                        val_neigh = assignments[m]
##                        try: csp.binary_constraints[n][m]
##                        except: continue
        ...
        def solve(self,max_iters=100):
                ...
                for _ in range(max_iters):
                        ...
                        c0,w0 = self.conflictedNeighbors(assignments,node)
                        ...
                        for u in D:
                                ...
                                c,w = self.conflictedNeighbors(assignments_cpy,node)
